...
def parse_arguments(block):
    ...
    if not len(glob.glob("raw/*")) and block != "export_session":
        if parameters["data_parent"] != None and not parameters["data_parent"] == ".":
            ...
            if not parameters["data_import"] and parameters["micromon_block"] != "tomo-denoising" and parameters["micromon_block"] != "tomo-picking-open" and parameters["micromon_block"] != "tomo-picking-closed":
