    ...
    if not os.path.isfile(micrographs):
        ...
        if parameters["data_mode"] == "tomo":
            if not parameters["movie_no_frames"]:
                if parameters["movie_source"] != "mdoc" and len(parameters["movie_pattern"]) > 0 and len(glob.glob("raw/*" + movie_extension)) > 0:
...
def inherit_from_parent(parameters,parameter_file):
    ...
    # start from the parent parameters
    clean_parameters = parameters.copy()
    
    # read parameters generated by the website
    parameters_from_file = toml.load(parameter_file)

    # read the specification file
    specifications = toml.load("/opt/pyp/config/pyp_config.toml")

    # override any parameters set by website or use default values if copyToNewBlock is False
    tabs = (t for t in specifications["tabs"].keys() if not t.startswith("_"))
    for t in tabs:
        ...
        for p in pars:
            ...
            if not "copyToNewBlock" in specifications["tabs"][t][p] or specifications["tabs"][t][p].get("copyToNewBlock") or k.startswith("csp_tomo_"):
                ...
                clean_parameters[k] = specifications["tabs"][t][p]["default"]
...
def parameter_force_check(previous_parameters, new_parameters, project_dir="."):
    ...
    if len(inputlist) and len(get_missing_files(previous_parameters, inputlist, exhaustive=False)) == 0 or new_parameters.get("micromon_block") == "tomo-import-pure":
