...
def get_gpu_info():
    try:
        ...
        if result != CUDA_SUCCESS:
            ...
            return 1
        logger.info("Found %d GPU device(s)" % nGpus.value)
        for i in range(nGpus.value):
            ...
            if result != CUDA_SUCCESS:
                ...
                if result == CUDA_SUCCESS:
                    logger.info(f"\tTotal Memory: {totalMem.value / 1024**3:.0f} GB")
                else:
                    cuda.cuGetErrorString(result, ctypes.byref(error_str))
                    logger.error(f"\tcuMemGetInfo failed with error code {result}: {error_str.value.decode()}")
