...
def sprtrain(args):
    ...
    mpi.submit_function_to_workers(bin_image, arguments)
...
def spreval(args,name):
    ...
    # bin data by 8
    bin_image( name + ".avg", name + "_bin.mrc", 8 )
    ...
    if 'detect_nn2d_ref' in args.keys() and os.path.exists( project_params.resolve_path(args['detect_nn2d_ref']) ):
        ...
        if args.get("detect_nn2d_debug"):
            ...
            for f in glob.glob( results_folder + "eval_imgs/*" ):
                logger.debug("Now saving " + Path(f).name + " to train/ folder")
...
def tomotrain(args):
    ...
    if args.get("detect_nn3d_milo_import") != "none":
        ...
        if args.get("detect_nn3d_milo_import") == "classes":
            if len(args.get("detect_nn3d_milo_classes").split(",")) == 0:
                ...
                # extract coordinates
                command = f"{NN_INIT_COMMANDS_3D} python -u {os.environ['PYP_DIR']}/external/cet_pick/cet_pick/utils/generate_train_file.py --dir {scratch_folder} --out {os.path.join(scratch_folder,'test')}"
                [ output, error ] = local_run.run_shell_command(command)
        ...
        # load classes from Phoenix
            ...
            if not os.path.exists( project_params.resolve_path(args["detect_nn3d_milo_parquet"])):
                ...
                command = f"{NN_INIT_COMMANDS_3D} python -u {os.environ['PYP_DIR']}/external/cet_pick/cet_pick/interactive_to_training_coords.py --input {project_params.resolve_path(args['detect_nn3d_milo_parquet'])} {if_double} --output {train_coords}"
                [ output, error ] = local_run.run_shell_command(command)
    ...
    if args.get('detect_nn3d_use_masking'):
        ...
        if args['detect_nn3d_mask_segmentation']:
            masking += "--mask_segmentation "
        if args['detect_nn3d_impute_tomograms']:
            masking += "--impute_tomograms "
        if args['detect_nn3d_mask_loss']:
            masking += "--mask_loss "
...
def tomoeval(args,name):
    ...
    if 'detect_nn3d_ref' in args.keys() and os.path.exists( project_params.resolve_path(args['detect_nn3d_ref']) ):
        ...
        command = f"{NN_INIT_COMMANDS_3D} python -u {os.environ['PYP_DIR']}/external/cet_pick/cet_pick/test.py semi --arch unet_4 --dataset semi {with_score} --exp_id test_reprod --load_model '{project_params.resolve_path(args['detect_nn3d_ref'])}' {compress} {gpu} {fiber} --down_ratio 2 --contrastive --translation_ratio {args['detect_nn3d_translation_ratio']} --K {args['detect_nn3d_max_objects']} --out_thresh {args['detect_nn3d_thresh']}  --patch_size {args['detect_nn3d_patch_size']} --patch_height {args['detect_nn3d_patch_height']} --loss_size_downscale {args['detect_nn3d_loss_size_downscale']} --loss_height_downscale {args['detect_nn3d_loss_height_downscale']} --pred_diameter {args['detect_nn3d_pred_diameter']} {masking}{compilation}--test_img_txt '{os.path.join( os.getcwd(), imgs_file)}' --test_coord_txt '{os.path.join( os.getcwd(), test_file)}' 2>&1 | tee '{os.path.join(project_folder, 'train', name + '_testing.log')}'"
        local_run.stream_shell_command(command)
...
def milotrain(args):
    ...
    if args.get('detect_milo_surface'):
        ...
        save_patches = "--save_patches " if args['detect_milo_save_patches'] else ""
        read_patches = f"--read_patches {args['detect_milo_read_patches']} " if 'detect_milo_read_patches' in args else ""

        if 'detect_milo_mode' in args and '2d' in args['detect_milo_mode']:
            command = (f"{NN_INIT_COMMANDS_3D} "
                       f"python -u {os.environ['PYP_DIR']}/external/cet_pick/cet_pick/simsiam_main.py simsiam2d3d "
                       f"--num_epochs {args['detect_milo_num_epochs']} "
                       f"--exp_id test_sample "
                       f"--bbox {args['detect_milo_bbox']} "
                       f"--dataset milopyp_surface2d3d "
                       f"--arch simsiam2d3d_{args['detect_milo_backbone']}  "
                       ...
                       f"--dog_std_multiplier {args['detect_milo_dog_std_multiplier']} "
                       f"--dog_diameter {args['detect_milo_dog_diameter']} "
                       f"--dog_boundary {args['detect_milo_dog_boundary']} "
                       f"{compress} "
                       f"{gpu} "
            ...
            command = (f"{NN_INIT_COMMANDS_3D} "
                       ...
                       f"--exp_id test_sample "
                       f"--bbox {args['detect_milo_bbox']} "
                       f"--dataset milopyp_surface3d "
                       f"--arch simsiam2d_{args['detect_milo_backbone']}  "
                       f"--lr {args['detect_milo_lr']} "
                       f"--train_img_txt {train_images} "
                       f"--batch_size {args['detect_milo_batch_size']} "
                       f"--val_intervals {args['detect_milo_val_interval']} "
    ...
    # copy saved patches to train folder
    for path in output_path.rglob('*.npz'):
        shutil.copy2( path, train_folder)
...
def miloeval(args):
    ...
    if 'detect_milo_model' in args.keys() and os.path.exists( project_params.resolve_path(args['detect_milo_model']) ):
        ...
        if args.get('detect_milo_surface'):
            ...
            if 'detect_milo_mode' in args and '2d' in args['detect_milo_mode']:
                command = (f"{NN_INIT_COMMANDS_3D} "
                           f"python -u {os.environ['PYP_DIR']}/external/cet_pick/cet_pick/simsiam_test_hm_2d3d.py simsiam2d3d "
                           f"--num_epochs {args['detect_milo_num_epochs']} "
                           f"--exp_id test_sample "
                           f"--bbox {args['detect_milo_bbox']} "
                           f"--dataset milopyp_surface2d3d "
