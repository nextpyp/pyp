...
def resize_template(mparameters, external_template, autopick_template):
    ...
    if (
        ...
        logger.debug(f"Rescaling template {external_template} {1/scaling:.2f}x to {model_pixel_size/scaling:.2f} A/pix")
...
def process_virions(
    ...
    if os.path.isfile("{0}.vir".format(name)):
        ...
        if len(arguments) > 0:
            ...
            mpi.submit_function_to_workers(process_virion_multiprocessing, arguments)
...
def detect_and_extract_particles( name, parameters, current_path, project_path, binning, x, y, zfact, tilt_angles, tilt_options, exclude_virions ):
    ...
    if virion_mode:
        ...
        if (
            ...
            or parameters["micromon_block"] == "" and not os.path.exists(f"{name}.spk") # sessioms have no block name defined
        ):
...
def extract_spk_direct(
    ...
    if os.path.isfile("%s.spk" % name):
        ...
        if parameters.get("micromon_block") not in [ "tomo-picking", "tomo-particles-eval", ""]:
            spikes = spikes[:, [0, 2, 1]]
