...
def cryocare(working_path, project_path, name, parameters):
    ...
    command = f"{command_base}cryoCARE_train.py --conf {train_config_file}"
    local_run.run_shell_command(command,verbose=parameters["slurm_verbose"])
...
def tomo_swarm_half(project_path, filename, keep=False):
    ...
    working_path = Path(os.environ["PYP_SCRATCH"]) / filename
    ...
    load_tomo_results(filename, parameters, current_path, working_path, verbose=parameters["slurm_verbose"])

    # unpack pkl file
    if os.path.exists(f"{filename}.pkl"):

        metadata_object = pyp_metadata.LocalMetadata(f"{filename}.pkl", is_spr=False)
    ...
    arguments = []
    for f in frame_list:
        arguments.append((str(current_path) + "/raw/" + f, f))
    ...
    for i, f in enumerate(raw_image):
        output_half1 = f.replace(filename, filename + "_half1").replace(".mrc", "")
    ...
    arguments = []
    for i, f in enumerate(raw_image):
        output_half2 = f.replace(filename, filename + "_half2").replace(".mrc", "")
    ...
    for i in [1, 2]:
        ...
        newfilename = filename + f"_half{i}"
        new_filelist = [file.replace(filename, newfilename) for file in raw_image]
        # copy the tilt aignment and angle files
        shutil.copy2(f"{filename}.xf", newfilename + ".xf")
        shutil.copy2(f"{filename}.tlt", newfilename + ".tlt")
        shutil.copy2(f"{filename}.order", newfilename + ".order")
        ...
        os.symlink(newfilename + ".mrc", newfilename + ".st")

        # actual stack sizes
        headers = mrc.readHeaderFromFile(newfilename + ".mrc")
        x = int(headers["nx"])
        y = int(headers["ny"])
        z = int(headers["nz"])
        ...
        generate_aligned_tiltseries(newfilename, parameters)

        # Refined tilt angles
        tltfile = f"{newfilename}.tlt"
        tilt_angles = np.loadtxt(tltfile) if os.path.exists(tltfile) else metadata["tlt"].to_numpy()

        exclude_views = merge.do_exclude_views(newfilename, tilt_angles)
