...
def align_tilt_series(name, parameters, rotation=0, excluded_views=""):
    ...
    if 'aretomo' == parameters["tomo_ali_method"]:
...
-Gpu {get_gpu_ids(parameters,separator=' ')}"
    ...
    # check if fiducial/patch tracking coordinates exist
    if 'aretomo3' == parameters["tomo_ali_method"]:
            ...
            patches_y = parameters["tomo_ali_patches_y"] if "tomo_ali_patches_y" in parameters else 1
            if patches_x + patches_y > 2:
                patches = f" -AtPatch {patches_x} {patches_y}"
            else:
                patches = ""

            # create sym link to rawtlt file
            os.symlink(f"{name}.rawtlt", f"{name}_aretomo.rawtlt")
...
-PixSize {parameters['scope_pixel']} {patches} \
...
-TmpDir {os.environ['PYP_SCRATCH']}"
            ...
            try:
                ...
                shutil.copy2(f"{name}.rawtlt", f"{name}.tlt")
                logger.error("aretomo3 failed to run")
